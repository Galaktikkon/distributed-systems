...
async def delete_and_error(error: int):
    ...
    if error >= 400 and error < 500:
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND, content=return_content
        )
    elif error >= 500 and error < 600:
        return JSONResponse(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, content=return_content
        )
    else:
        return JSONResponse(
            status_code=status.HTTP_501_NOT_IMPLEMENTED, content=return_content
        )
